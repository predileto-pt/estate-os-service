...
async def test_init_mints_cookie_and_anonymous_view(client):
    response = await client.post("/api/v1/portal/session/init")
...
async def test_init_with_valid_cookie_is_no_op(client):
    r1 = await client.post("/api/v1/portal/session/init")
    assert r1.status_code == 200
    r2 = await client.post("/api/v1/portal/session/init")
...
async def test_me_without_cookie_auto_mints(client):
    # `/me` is the FE bootstrap endpoint — no prior `/init` required.
    # First hit mints a fresh anonymous session and sets the cookie.
    r = await client.get("/api/v1/portal/session/me")
...
async def test_patch_rejects_oversized_prefs(client):
    await client.post("/api/v1/portal/session/init")
    big = {"k": "x" * 9000}
    r = await client.patch("/api/v1/portal/session/me", json={"prefs": {"merge": big}})
    assert r.status_code == 400
...
async def test_claim_with_valid_token_flips_to_authenticated(client):
    ...
    pid = str(uuid4())
    await client.patch(
        "/api/v1/portal/session/me", json={"favorites": {"add": [pid], "remove": []}}
    )
...
async def test_claim_without_bearer_header_returns_401(client):
    await client.post("/api/v1/portal/session/init")
    r = await client.post("/api/v1/portal/session/claim")
    assert r.status_code == 401
    assert r.json()["code"] == "PORTAL_AUTH_TOKEN_INVALID"
...
async def test_logout_is_idempotent_on_anonymous_session(client):
    await client.post("/api/v1/portal/session/init")
    r1 = await client.post("/api/v1/portal/session/logout")
    r2 = await client.post("/api/v1/portal/session/logout")
    assert r1.status_code == 200
    assert r2.status_code == 200
