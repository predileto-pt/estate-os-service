...
**Two parallel Terraform stacks.** `terraform/production-coolify/` is the active AWS surface for the Coolify path — owns the ECR repo, the `documents` S3 bucket, the `images` S3 bucket (public-read for objects, fronted by Cloudflare at `https://images.predileto.pt` via a proxied CNAME + Origin Rule Host-header rewrite — Cloudflare terminates TLS, S3 serves the bytes directly), the GitHub OIDC role for CI image push, and two IAM users (one for the VM host's `docker login` refresh, one for the api container's S3 calls on both buckets). `terraform/production/` is the dormant Lambda/EC2/ALB/VPC code retained as the emergency revert path; its AWS resources are deleted. CI lives in `.github/workflows/co-build-and-push.yml` (OIDC → ECR → project-level Coolify webhook); the legacy `deploy.yml` is `workflow_dispatch`-only. The VM pulls from ECR via a systemd timer (`ecr-login.timer`) that refreshes `docker login` every 8h using the static keys of the `coolify-ecr-reader` IAM user — Coolify uses the host's docker daemon, so no registry credentials live in the Coolify UI. **Migrations run automatically** on every Coolify deploy via the `migrations` one-shot service in `deploy/docker-compose.prod.yml`; the api + workers `depends_on.migrations: service_completed_successfully` so they refuse to start against a stale schema. **Property images** upload via presigned PUT directly to S3 (bucket CORS allows GET/PUT/HEAD from production + dev origins) and serve over `https://images.predileto.pt` — `RecordPropertyImage` derives the URL from `IMAGES_CDN_BASE_URL` + s3_key (LocalStack dev falls back to the LocalStack endpoint URL when the env var is empty). Bring-up walkthrough: `docs/runbooks/coolify-first-deploy.md`.
